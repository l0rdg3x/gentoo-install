...
- **LUKS full-disk encryption** with optional **TPM2 auto-unlock** (PCR 7+14)
...
This binds the LUKS key to TPM2 PCR 7+14 (Secure Boot state + shim/MOK policy). Your LUKS passphrase continues to work as a fallback.
...
| **TPM2 auto-unlock** | systemd: `systemd-cryptenroll` / OpenRC: `clevis` — binds LUKS to PCR 7+14 (Secure Boot state + shim/MOK policy) |
